...
import com.KafkaConsumer_Storm.config.ApplicationConfiguration;
...
public class Spout extends BaseRichSpout{
	...
	public void open(Map map, TopologyContext arg1, SpoutOutputCollector collector) {
		System.out.println("open is triggered");
		...
		for (;;) {
			try {
				...
				if (null != msgList && !msgList.isEmpty()) {
					String msg = "";
					List<DeptGet> list=new ArrayList<DeptGet>();
					for (ConsumerRecord<String, String> record : msgList) {
						// 原始数据
						msg = record.value();
						logger.error("来自kafka的数据:"+msg);
						if (null == msg || "".equals(msg.trim())) {
							continue;
						}
						try{
							list.add(JSON.parseObject(msg, DeptGet.class));
						}catch(Exception e){
